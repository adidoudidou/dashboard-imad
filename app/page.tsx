    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', fontFamily: 'Inter, sans-serif', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '16px 14px', maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {tab === 'overview' && <>
          {/* KPIs 2x2 */}
          <div className="grid2">
            <KpiCard label="Revenus" value={eur(d.totalRevenus)} sub={`${eur(d.revenusMoisCourant)} ce mois`} color="#22D3A5" icon="💰" />
            <KpiCard label="Dépenses" value={eur(d.totalDepenses)} sub={`${eur(d.depensesMoisCourant)} ce mois`} color="#F43F5E" icon="📉" />
            <KpiCard label="Bénéfice" value={eur(benefice)} sub={`${eur(beneficeMois)} ce mois`} color={benefice >= 0 ? '#22D3A5' : '#F43F5E'} icon="✨" />
        ...
        {tab === 'depenses' && <>
          <div className="grid2">
            ...
            <KpiCard label="Ce mois" value={eur(d.depensesMoisCourant)} color="#F43F5E" icon="📅" />
          </div>
